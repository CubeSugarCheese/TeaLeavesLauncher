    ...
    def _generate_launch_parameter(self):
        ...
        if self.user_type == "Legacy":
            part_game = f"--username {self.name} --version {self.version} --gameDir \"{self.mc_path}\\versions\\{self.version}\" " + \
                        ...
                        f"--userType mojang --uuid {self.uuid} --accessToken f266a689f9b1424995bc341fc0966560 --versionType TeaLeavesLauncher " + \
            ...
            part_game = f"--username {self.name} --version {self.version} --gameDir \"{self.mc_path}\\versions\\{self.version}\" " + \
                        ...
                        f"--width {self.width} --height {self.height}"
        return part_X + part_D + part_cp + part_class + part_game
